...
function doGet() {
    return HtmlService.createTemplateFromFile('TimeTracker').evaluate()
...
function clockIn(employee) {
  ...
  for (var j = 2; j <= lastRow; j++)
  {
    ...
    {
      error = '<br> คุณต้องตอกบัตรเลิกงานก่อน';
...
function getDate(return_array)
{
  ...
  var date =  currentDate.getDate()+ '/' + currentMonth.toString().toString() + '/' + 
             currentYear.toString() + '<br> เวลา ' + currentHours.toString() + ':' +
...
function clockOut(employee) {
  ...
  {
    return_array.push(['<br> ต้องตอกบัตรเข้างานก่อน ', '', employee]);
    return return_array; 
  }
